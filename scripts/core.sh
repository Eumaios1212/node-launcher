...
get_node_service() {
  ...
  menu thornode thornode bifrost midgard gateway binance-smart-daemon dogecoin-daemon gaia-daemon avalanche-daemon ethereum-daemon bitcoin-daemon litecoin-daemon bitcoin-cash-daemon xrp-daemon midgard-timescaledb
...
generate_mnemonic() {
  ...
  mnemonic=$(kubectl exec -n "$NAME" mnemonic -- thornode keys add generate --dry-run --output json | jq -r '.mnemonic')
  [[ $mnemonic == "" ]] || [[ $mnemonic == "null" ]] && die "Mnemonic generation failed. Please try again."
